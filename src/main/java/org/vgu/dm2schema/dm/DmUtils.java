...
public class DmUtils {
    ...
    public static String getAssociationEndTargetClassName(
            DataModel dm, String className, String endName) {
        End end = getEnd(dm, className, endName);
        return Optional.ofNullable(end).map(End::getTargetClass).orElse(null);
    }

    public static String getAssociationEndSourceClassName(
            DataModel dm, String className, String endName) {
        End end = getEnd(dm, className, endName);
    ...
    public static boolean isEndMultOne(DataModel dm, String className, String endName) {
        ...
        return Optional.ofNullable(end)
                .map(End::getMult)
                .map(v -> v == Multiplicity.ONE)
                .orElse(false);
    }

    private static AssociationType getAssocRelationship(
            DataModel dm, String className, String endName) {
    ...
    private static AssociationType getRelationshipType(
            Multiplicity leftMult, Multiplicity rightMult) {
        boolean isLeftMultOne = leftMult == Multiplicity.ONE;
        boolean isRightMultOne = rightMult == Multiplicity.ONE;

        if (isLeftMultOne ^ isRightMultOne) return AssociationType.MANY_TO_ONE;
        return AssociationType.ONE_TO_ONE;
        // if (isLeftMultOne && isRightMultOne) return AssociationType.ONE_TO_ONE;
